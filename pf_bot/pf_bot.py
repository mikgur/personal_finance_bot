...
def run_bot():
    ...
    dp.add_handler(MessageHandler(Filters.regex(AMOUNT_PATTERN), handlers.add_transaction))
    dp.add_handler(categories_menu.conversation)
    dp.add_handler(RegexHandler(make_re_template_for_menu(main_menu.statistics), handlers.show_statistics))
