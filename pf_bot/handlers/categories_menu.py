...
from pf_bot.utils import (
    clear_user_data, confirmation, get_keyboard, get_keyboard_from_list,
    main_menu, make_re_template_for_menu
)
...
Menu = namedtuple(
    "Menu",
    ["expense_add", "expense_remove", "income_add", "income_remove", "back"]
)
action_choices = Menu(
    "Расходы – Добавить категорию", "Расходы – Удалить категорию",
    "Доходы – Добавить категорию", "Доходы – Удалить категорию", "Назад"
)
...
def start(bot, update):
    update.message.reply_text(
        ...
        reply_markup=get_keyboard_from_list(
            list(action_choices), cancel=False, one_time_keyboard=True
        )
    )
    return "menu_choice"
...
def menu_choice(bot, update, user_data):
    ...
    if choice == action_choices.back:
        update.message.reply_text(
            f"{user.first_name}, слушаю тебя",
            reply_markup=get_keyboard("main_menu")
        )
        clear_user_data(user_data, "categories_menu")
        return ConversationHandler.END

    category_type = "income" if choice in [
        action_choices.income_add, action_choices.income_remove
    ] else "expense"
    action = "add" if choice in [
        action_choices.income_add, action_choices.expense_add
    ] else "remove"
...
def add_category(bot, update, user_data):
    ...
    try:
        ...
        category_added = data_manipulator.add_category(
            new_category, user.id, user_data["add_category_type"]
        )
        if category_added:
        ...
        update.message.reply_text(
            reply_text, reply_markup=get_keyboard("main_menu")
        )
        clear_user_data(user_data, "categories_menu")
        return ConversationHandler.END
        ...
        update.message.reply_text(
            "не получается определить тип категории, \
                выбери еще раз желаемое действие",
            reply_markup=get_keyboard("main_menu")
        )
        clear_user_data(user_data, "categories_menu")
        return ConversationHandler.END
        ...
        reply_text = "Такая категория уже существует, кстати, вот список всех \
            категорий, которые у тебя есть:"

        for i, category in enumerate(
            sorted(
                data_observer.get_all_category_names(
                    user.id, user_data["add_category_type"], status="active"
                )
            )
        ):
...
def delete_category(bot, update, user_data):
    ...
    #  Get necessary information and check whether the category_name is
    #  correct. In case everythin is ok ask for confirmation
    categories = data_observer.get_all_category_names(
        user.id, user_data["delete_category_type"]
    )
    if category_name in categories:
        user_data["delete_category_name"] = category_name
        update.message.reply_text(
            f"Уверен, что хочешь удалить {category_name}?",
            reply_markup=get_keyboard("confirmation", one_time_keyboard=True)
        )
        return "confirm_delete_category"
    else:
        update.message.reply_text(
            f"Не могу найти {category_name}, \
                выбери категорию при помощи клавиатуры",
            reply_markup=get_keyboard_from_list(
                categories, one_time_keyboard=True
            )
        )
...
def confirm_delete_category(bot, update, user_data):
    user = update.message.from_user
    choice = update.message.text
    #  Lets check whether everything is correct
    #  (category_name and category_type)
    if ("delete_category_name" in user_data
            and "delete_category_type" in user_data):
        ...
        if choice.lower() == confirmation.yes.lower():
            if data_manipulator.delete_category(
                user.id, category_name, category_type
            ):
        ...
        #  user reply is neither yes nor no - wrong answer
            ...
            category_name = user_data["delete_category_name"]
            reply_text = f"Не понимаю твой ответ. Ты уверен, \
                что хочешь удалить категорию {category_name}?"
            update.message.reply_text(
                reply_text,
                reply_markup=get_keyboard(
                    "confirmation", one_time_keyboard=True
                )
            )
            return "confirm_delete_category"
    else:
...
conversation = ConversationHandler(
    ...
    states={
        "menu_choice": [
            RegexHandler(
                make_re_template_for_menu(list(action_choices)),
                menu_choice,
                pass_user_data=True
            )
        ],
        ...
        "confirm_delete_category": [
            MessageHandler(
                Filters.text, confirm_delete_category, pass_user_data=True
            )
        ],
        "add_category":
        [MessageHandler(Filters.text, add_category, pass_user_data=True)]
    },
    fallbacks=[]
)
