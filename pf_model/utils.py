...
def get_category_type_by_alias(alias):
    """In database categories types are "expense" and "income".
    But telegram bot knows nothing about it and will try
    to send category types as expense/расходы or income/доходы due to
    ...
    if alias.lower() in ["expense", "расходы"]:
        ...
        raise WrongCategoryType(
            f"There is no such category class in database: {alias}"
        )
